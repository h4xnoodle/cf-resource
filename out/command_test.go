...
var _ = Describe("Out Command", func() {
	...
	Context("Multi Buildpack support", func() {
		BeforeEach(func() {
			cloudFoundry = &fakes.FakePAAS{}
			command = out.NewCommand(cloudFoundry)
			...
			request = out.Request{
				...
				Params: out.Params{
					MultiBuildpacks: []string{"ruby_buildpack", "java_buildpack"},
					Path: "build/app.jar",
		...
		It("pushes with buildpacks ordered correctly", func() {
			response, err := command.Run(request)
			Expect(err).NotTo(HaveOccurred())
			...
			buildpacks, path, currentAppName := cloudFoundry.V3PushAppArgsForCall(0)
			Expect(path).To(Equal("build/app.jar"))
			Expect(currentAppName).To(Equal("awesome-app"))
			Expect(buildpacks).To(Equal([]string{"ruby_buildpack", "java_buildpack"}))
		})
	})
